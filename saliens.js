...
class BotGUI {
	...
	createStatusWindow() {
		...
		var $statusWindow = $J([
			'<div id="salienbot_gui" style="background: #191919; z-index: 1; border: 3px solid #83d674; padding: 20px; margin: 15px; width: 300px; transform: translate(0, 0);">',
				...
				'<p><b>Seviye:</b> <span id="salienbot_level">' + this.state.level + '</span> &nbsp;&nbsp;&nbsp;&nbsp; <b>Seviye Sayısı:</b> <span id="salienbot_exp">' + this.state.exp + " / " + this.state.next_level_exp + '</span></p>',
				'<p><b>Seviye Atlamaya;:</b> <span id="salienbot_esttimlvl"></span></p>',
				'<p><input id="planetSwitchCheckbox" type="checkbox"/> Otomatik Gezegen değiştirme</p>',
				'<p><input id="animationsCheckbox" type="checkbox"/> Oyunu Gizle (Daha iyi performans için.)</p>',
			'</div>'
		].join(''))
	...
	updateStatus(running) {
		const statusTxt = running ? '<span style="color: green;">✓ Çalışıyor...</span>' : '<span style="color: red;">✗ Durdu!</span>';
		...
		success: function(data, textStatus, jqXHR) {
			if( $J.isEmptyObject(data.response) ) {
				...
				if(window.gGame.m_State.m_Grid.m_Tiles[zone].Info.captured || attempt_no >= max_retry) {
					...
					gui.updateTask("5 saniye bekleyin. Istek Tekrar gönderiliyor. (Attempt #" + (attempt_no + 1) + ").");
